...
MOVIE_DATABASE = {k.lower(): v for k, v in MOVIE_DATABASE.items()}
...
def classify_mood(acousticness, danceability, energy, speechiness, valence, tempo):
    ...
    scores['Feliz'] = (valence * 0.4) + (energy * 0.3) + (danceability * 0.3)
    scores['Triste'] = ((1 - valence) * 0.4) + ((1 - energy) * 0.3) + (acousticness * 0.2)
    scores['Enérgico'] = (energy * 0.4) + (tempo / 200 * 0.3) + (danceability * 0.3)
    scores['Calmado'] = ((1 - energy) * 0.4) + (acousticness * 0.3) + ((1 - speechiness) * 0.2)
    scores['Tenso'] = ((1 - danceability) * 0.3) + ((1 - valence) * 0.3) + (energy * 0.3)
    scores['Romántico'] = (valence * 0.4) + ((1 - energy) * 0.3) + ((1 - danceability) * 0.2)
    scores['Nostálgico'] = (acousticness * 0.5) + ((1 - energy) * 0.3) + (valence * 0.2)
    scores['Confianza'] = (energy * 0.4) + (valence * 0.3) + (danceability * 0.3)
    scores['Melancolía'] = (acousticness * 0.3) + ((1 - valence) * 0.4) + ((1 - energy) * 0.3)
    scores['Eufórico'] = (energy * 0.35) + (valence * 0.35) + (danceability * 0.3)
    scores['Aburrido'] = ((1 - energy) * 0.4) + ((1 - valence) * 0.3) + ((1 - danceability) * 0.3)
    scores['Agresivo'] = (energy * 0.4) + (speechiness * 0.3) + ((1 - acousticness) * 0.3)
    scores['Soñador'] = (acousticness * 0.4) + ((1 - energy) * 0.3) + ((1 - danceability) * 0.3)
    scores['Misterioso'] = (acousticness * 0.4) + ((1 - speechiness) * 0.3) + ((1 - valence) * 0.2)
    scores['Juguetón'] = (danceability * 0.4) + (valence * 0.3) + (energy * 0.3)
